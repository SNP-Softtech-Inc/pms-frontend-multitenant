...
const ServicesComponent = ({
  ...
  const validateItemizedData = () => {
    ...
    if (!itemizedData.rows || itemizedData.rows.length === 0) {
      ...
      const rowErrors = itemizedData.rows
        .map((row, index) => {
          const rowError = {};
  ...
  const clearRowErrors = (rowIndex) => {
    if (stepErrors.rowErrors) {
      setStepErrors((prev) => {
        const newErrors = { ...prev };
        newErrors.rowErrors = newErrors.rowErrors.filter(
  ...
  const deleteRow = (rowIndex) => {
    const updatedRows = itemizedData.rows.filter(
  ...
  const fetchservicebyid = async (id, rowIndex) => {
    const SERVICE_API =
      process.env.REACT_APP_SERVICE_API || "https://www.snptaxes.com";
  ...
  const handleTaxRateChange = (e) => {
    ...
    const subtotal = itemizedData.rows.reduce(
      (sum, row) => sum + (parseFloat(row.amount) || 0),
      0,
    );
  ...
  const getRowError = (rowIndex, field) => {
    if (stepErrors.rowErrors) {
      const rowError = stepErrors.rowErrors.find(
        (error) => error.rowIndex === rowIndex,
      );
  ...
  const CATEGORY_API =
    process.env.REACT_APP_CATEGORY_URL || "https://www.snptaxes.com";
      ...
      <Box sx={{ mt: 3 }}>
        ...
        <Box sx={{ overflow: "auto", width: "100%" }}>
          <Table>
            ...
            <TableBody>
              {itemizedData.rows &&
                itemizedData.rows.map((row, rowIndex) => (
                  <TableRow key={rowIndex}>
                    ...
                    <TableCell>
                      ...
                      >
                        <MenuItem
                          onClick={() => handleEditService(row, rowIndex)}
                        >
                          Edit
                        </MenuItem>
