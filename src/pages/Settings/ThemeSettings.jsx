...
// import { useState, useEffect, useCallback } from "react";
// import { Check, RotateCcw, ChevronDown, ChevronUp, Type } from "lucide-react";
// import { toast } from "react-toastify";

// // ─── Theme definitions with base typography configurations ───────────────────
// // const THEMES = {
// //   default: {
// //     label: "Ocean Blue", emoji: "🌊", description: "Your current default",
// //     vars: {
...
// //       "--border": "214 20% 88%", "--input": "214 20% 88%",
// //       "--ring": "217 89% 55%", "--popover": "0 0% 100%", "--popover-foreground": "222 47% 11%",
// //       "--font-family": "'Inter', sans-serif", "--font-scale": "100%"
// //     },
// //   },
// //   violet: {
// //     label: "Violet Dusk", emoji: "🔮", description: "Deep purple tones",
...
// // ─── Theme definitions with updated defaults ───────────────────
// const THEMES = {
//   default: {
//     label: "Ocean Blue", emoji: "🌊", description: "Your current default",
//     vars: {
...
//       "--border": "214 20% 88%", "--input": "214 20% 88%",
//       "--ring": "217 89% 55%", "--popover": "0 0% 100%", "--popover-foreground": "222 47% 11%",
//       // Changed default variable assignment here:
//       "--font-family": "'Geist', sans-serif", "--font-scale": "100%"
//     },
//   },
//   violet: {
//     label: "Violet Dusk", emoji: "🔮", description: "Deep purple tones",
...
// // Added Geist option here to the typography picker options array
// const FONT_OPTIONS = [
//   { value: "'Geist', sans-serif", label: "Geist (Vercel Clean)" },
//   { value: "'Inter', sans-serif", label: "Inter (Classic Sans)" },
//   { value: "'Roboto', sans-serif", label: "Roboto (Modern Tech)" },
...
// const SCALE_OPTIONS = [
//   { value: "93.75%", label: "Small (Compact)" },
//   { value: "100%", label: "Medium (Default)" },
//   { value: "106.25%", label: "Large (Readable)" },
//   { value: "112.5%", label: "Extra Large" }
// ];

// // ─── Utilities ────────────────────────────────────────────────────────────────
// const hsl = (v) => v?.includes("%") ? v : `hsl(${v})`;
...
// function hslToHex(hslStr) {
//   if (!hslStr || hslStr.includes("'") || hslStr.includes("%")) return "#6b7280";
//   try {
//     const [h, s, l] = hslStr.trim().split(/\s+/).map(parseFloat);
//     const c = document.createElement("canvas");
//     c.width = c.height = 1;
...
//         <div className="h-2 w-3/4 rounded" style={{ background: hsl(vars["--foreground"]) }} />
//         <div className="h-1.5 w-1/2 rounded" style={{ background: hsl(vars["--muted-foreground"]) }} />
//       </div>
//     </div>
//   );
// }
...
//       onMouseLeave={onMouseLeave}
//       className={[
//         "relative text-left rounded-xl border-2 p-3 w-full transition-all duration-150 bg-card",
//         isActive ? "border-primary shadow-md" : "border-border hover:border-primary/40"
//       ].join(" ")}
//     >
//       <MiniPreview vars={theme.vars} />
//       <p className="text-xs font-semibold text-foreground">{theme.emoji} {theme.label}</p>
//     </button>
//   );
// }

// // ─── Custom Font and Scale Sub-Editor ──────────────────────────────────────────
// function TypographyEditor({ baseVars, onChange }) {
//   const currentFont = baseVars["--font-family"] || "'Inter', sans-serif";
//   const currentScale = baseVars["--font-scale"] || "100%";

//   const updateTypo = (key, value) => {
//     onChange({ ...baseVars, [key]: value });
//   };

//   return (
//     <div className="border rounded-xl p-4 bg-secondary/30 border-border space-y-4">
//       <div className="flex items-center gap-2 font-medium text-sm text-foreground mb-1">
//         <Type className="w-4 h-4 text-primary" />
//         <h4>Typography Framework</h4>
//       </div>

//       <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//         {/* Font Family Selection */}
//         <div className="flex flex-col gap-1.5">
//           <label className="text-xs font-medium text-muted-foreground">Text Font Family</label>
//           <select 
//             value={currentFont}
//             onChange={(e) => updateTypo("--font-family", e.target.value)}
//             className="w-full text-xs rounded-lg border border-input bg-card p-2 text-foreground focus:ring-2 focus:ring-ring focus:outline-none"
//           >
...
//         <div className="flex flex-col gap-1.5">
//           <label className="text-xs font-medium text-muted-foreground">Global Scaling Multiplier</label>
//           <select 
//             value={currentScale}
//             onChange={(e) => updateTypo("--font-scale", e.target.value)}
//             className="w-full text-xs rounded-lg border border-input bg-card p-2 text-foreground focus:ring-2 focus:ring-ring focus:outline-none"
//           >
...
//       {/* Live Preview Elements inside panel */}
//       <div className="mt-3 p-3 rounded-lg border bg-card border-border space-y-1.5">
//         <span className="text-[10px] uppercase tracking-wider font-semibold text-muted-foreground">Live Scaling Preview</span>
//         <h1 className="text-xl font-bold text-foreground">Heading Level 1 Example</h1>
//         <p className="text-sm text-muted-foreground">
//           Paragraph text example block. Modifying options above applies structural transformations automatically across system panels.
//         </p>
//       </div>
//     </div>
//   );
...
// function CustomEditor({ baseVars, onChange }) {
//   const handlePick = (key, hex) => {
//     onChange({ ...baseVars, [key]: hexToHsl(hex) });
//   };

//   return (
//     <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
//       {EDITABLE_VARS.map(({ key, label }) => (
//         <div key={key} className="flex items-center gap-3 rounded-lg border px-3 py-2 bg-card border-border">
//           <label className="relative w-8 h-8 rounded-md overflow-hidden cursor-pointer border shrink-0">
//             <div className="w-full h-full" style={{ background: hsl(baseVars[key]) }} />
//             <input
//               type="color"
//               className="absolute inset-0 opacity-0 cursor-pointer"
//               value={hslToHex(baseVars[key])}
//               onChange={(e) => handlePick(key, e.target.value)}
//             />
//           </label>
//           <div className="min-w-0">
...
// // ─── Shared UI wrappers matching framework design systems ───────────────────
// const SettingsCard = ({ title, children }) => (
//   <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
//     <div className="border-b border-border px-5 py-3.5">
//       <h3 className="text-sm font-semibold text-foreground">{title}</h3>
...
//       localStorage.removeItem(CUSTOM_KEY);
//     }
//     toast.success("Theme configurations updated!");
//   };

//   return (
//     <div className="space-y-6">
//       <SettingsCard title="Visual presets">
//         <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
//           {Object.entries(THEMES).map(([key, theme]) => (
//             <ThemeCard
//               key={key} id={key} theme={theme}
//               isActive={activeKey === key && !isDirty}
//               onSelect={handleSelectPreset}
//               onMouseEnter={(k) => applyVars(THEMES[k].vars)}
//               onMouseLeave={() => applyVars(customVars ?? committedVars)}
//             />
//           ))}
//         </div>
//         <div className="pt-2">
...
//       <SettingsCard title="Typography & Micro-Modifications">
//         <button
//           onClick={() => setShowAdvanced((v) => !v)}
//           className="flex items-center gap-2 text-sm font-medium text-foreground hover:text-primary transition-colors"
//         >
//           {showAdvanced ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
//           Advanced Design Framework Settings
//         </button>

//         {showAdvanced && (
//           <div className="space-y-6 pt-2 form-section-enter">
//             {/* Added Typography Sub-module here */}
//             <TypographyEditor 
//               baseVars={customVars ?? committedVars}
//               onChange={handleCustomChange}
//             />
...
//             <div>
//               <label className="text-xs font-semibold text-foreground uppercase tracking-wider block mb-2">Palette Overrides</label>
//               <CustomEditor
//                 baseVars={customVars ?? committedVars}
//                 onChange={handleCustomChange}
//               />
...
//               <SaveBtn onClick={handleSave} />
//               {isDirty && (
//                 <button onClick={handleResetCustom} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
//                   <RotateCcw className="w-3.5 h-3.5" /> Reset Variables
//                 </button>
//               )}
//             </div>
//           </div>
...
const THEMES = {
  default: {
    label: "Ocean Blue",
    emoji: "🌊",
    description: "Your current default",
    vars: {
      "--primary": "217 89% 61%",
      "--primary-foreground": "0 0% 100%",
...
function hslToHex(hslStr) {
  if (
    !hslStr ||
    hslStr.includes("'") ||
    hslStr.includes("sans") ||
    hslStr.includes("serif")
  ) {
    return "#6b7280";
  }
...
function MiniPreview({ vars }) {
  return (
    <div
      className="rounded-lg overflow-hidden border mb-2.5"
      style={{
    ...
    >
      ...
      <div className="p-2 flex flex-col gap-1">
        <div
          ...
          style={{
            background: hsl(vars["--foreground"]),
          }}
        />
...
function TypographyEditor({ baseVars, onChange }) {
  const currentFont =
    baseVars["--font-family"] ||
    "'Geist', sans-serif";

  const currentScale =
    baseVars["--font-scale"] || "100%";
  ...
  return (
    <div className="border rounded-xl p-4 bg-secondary/30 border-border space-y-4">
      ...
      <div className="mt-3 p-3 rounded-lg border bg-card border-border space-y-1.5">
        ...
        <p className="text-sm text-muted-foreground">
          Paragraph text example block.
          Modifying options above applies
          structural transformations
          automatically across system
          panels.
        </p>
      </div>
    </div>
...
export default function ThemeSettings() {
  ...
  return (
    <div className="space-y-6">
      ...
      <SettingsCard title="Colour theme">
        <p className="text-sm text-muted-foreground -mt-1">
          Choose a colour theme applied
          across buttons, backgrounds,
          and cards. Hover to preview —
